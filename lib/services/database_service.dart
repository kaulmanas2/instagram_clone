...
class DatabaseService {
  ...
  final CollectionReference userPostsCollection =
  ...
  Future updateUserProfilePicture(File profilePic) async {
    try {
      Reference ref =
          FirebaseStorage.instance.ref(uid).child("${uid}_profile_pic");
      UploadTask uploadTask = ref.putFile(profilePic);
      uploadTask.whenComplete(() async {
        String downURL = await uploadTask.snapshot.ref.getDownloadURL();
        return await userProfilesCollection
            .doc(uid)
            .update({"profile_pic": downURL});
      });
    } catch (e) {
      print(e.toString());
    }
